...
impl fmt::Display for ModelLocalPaths {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "ModelLocalPaths {{ model_full_path: {} }}",
            self.model_full_path.to_string_lossy(),
        )
    }
}
...
pub async fn download_model_with_config(
    ...
    log::info!("model_root: {}", model_root.to_string_lossy());

    Ok(ModelLocalPaths {
        model_full_path: model_root,
    })
}
