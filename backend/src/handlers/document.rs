...
pub async fn add_document(
    ...
    tokio::spawn(async move {
        ...
        match vectorize(&data.config.embedder, chunks, &data.local_embedder).await {
...
pub async fn import_documents(
    ...
    tokio::spawn(async move {
        ...
        for (index, result) in results.into_iter().enumerate() {
            ...
            let local_embedder = data.local_embedder.clone();
            preprocess_tasks.push(tokio::spawn(async move {
                ...
                document_metadata.chunks =
                    vectorize(&embedder_config, chunks, &local_embedder).await?;
...
pub async fn update_document_content(
    ...
    tokio::spawn(async move {
        ...
        metadata.chunks = match vectorize(&data.config.embedder, chunks, &data.local_embedder).await
        {
            Ok(chunks) => chunks,
            Err(error) => {
                error!("Failed to vectorize document chunks: {}", error);
    ...
    tokio::spawn(async move {
        ...
        for (collection_metadata_id, mut document_metadata, document_content) in results {
            ...
            slicing_tasks.push(tokio::spawn(async move {
                ...
                let chunks = vectorize(&embedder_config, chunks, &local_embedder).await?;
